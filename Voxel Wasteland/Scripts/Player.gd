...
export var walk_force = 100 # N (kg*m/s^2)
export var walk_speed = 5 # m/s
export var jetpack_up_force = 30
...
func jetpack_move(delta):
	...
	if jetpack_active:
		...
		if Input.is_action_pressed("jump"):
			...
			change_momentum(Vector3.UP * jetpack_up_force * delta)
		change_momentum(horizontal_input_direction * jetpack_side_force * delta)
...
func _physics_process(delta):
	...
	# TODO: MAX walk speed with walk force (to allow for stopping power)
	if is_on_floor():
		...
		if (horizontal_velocity.length_squared() < (walk_speed * walk_speed)):
			change_momentum(horizontal_input_direction * walk_force * delta)
